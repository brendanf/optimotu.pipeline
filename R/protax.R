...
#' @param seqs_file (`character` vector) optional per-file paths overriding
#' those stored in the index, if `seqs` is a
...
#' @return a `character` vector of output files generated by protax
#' @export
run_protax <- function(
  seqs,
  outdir,
...
parse_protax_nameprob <- function(nameprob, id_is_int = FALSE) {
  ...
  `names<-`(nameprob, basename(nameprob)) |>
    ...
    dplyr::mutate(
      !!id_col_name := if (id_is_int) {
        as.integer(!!id_col_name)
      } else {
        !!id_col_name
      },
...
run_protax_animal <- function(
  ...
  if (strip_inserts | is_gz) {
    ...
    if (strip_inserts) {
      pipecommand <- paste(pipecommand, "| tr -d 'acgt'")
    }
...
run_protax_besthit <- function(
  ...
  for (i in seq_len(n_query)) {
    besthit[[i]]$wait()
    besthit_exit_status <- max(
      besthit_exit_status,
      besthit[[i]]$get_exit_status()
    )
...
#' @export
protax_besthit_closedref <- function(
  infile,
  index,
  i,
  unknowns,
...
seq_cluster_protax <- function(aln_seq, aln_index, which, thresh, aln_len) {
  ...
  optimotu::distmx_cluster(
    ...
    parallel_config = optimotu::parallel_concurrent(max(
      1,
      local_cpus() - nslice
    ))
  )
...
parse_protaxAnimal_output <- function(x) {
  ...
  if (is.integer(out$seq_id)) {
    out <- dplyr::rename(out, seq_idx = seq_id)
  }
  out
}
